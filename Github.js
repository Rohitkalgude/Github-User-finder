...
async function fetchRepositories(respsurl) {
  reposContainer.innerHTML = `<div class="loading-repos">Loading repositories...</div>`;

  try {
    const response = await fetch(`${respsurl}?per_page=20`);
...
function displayRepos(repos) {
  if (repos.length === 0) {
    reposContainer.innerHTML = `<div class="no-repos">No repositories</div>`;
    return;
  }
  ...
  <div class="repo-meta">
    ...
    <div class="repo-meta-item">
      <i class="fas fa-history"></i> ${updateAt}
    </div>
  </div>
`;
...
searchInput.value = "rohitkalgude";
searchUser();
